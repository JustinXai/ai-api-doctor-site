...
function buildDebugScoring(rawScore, cappedScore, checks, breakdown, extra = {}) {
  ...
  return {
    ...
    scoreSourceVersion: 'v1.11.12-real-render-source',
...
function buildModuleScores(checks, locale) {
  ...
  // v1.11.12 calibration: targetCall success + OpenAI-compatible response → 23/25
  if (realTargetCallSuccess) {
    if (targetCallEvidence.openAICompatible && targetCallEvidence.hasContent) {
      // C. OpenAI-compatible response (hasChoices or hasMessage) + content readable → 23/25
      if (rawBasicScore < 23) {
        basicScore = 23;
        basicReason = 'openai_compatible_response';
        basicSource = 'v11112_calibration: openai compatible → 23';
      }
    } else if (rawBasicScore < 20) {
      // B. Minor issues, keep at minimum 20
      basicScore = 20;
      basicReason = 'minor_compatibility_issues';
      basicSource = 'v11112_calibration: minor issues → 20';
    }
  }
  // A. targetCall failed → keep raw score (will be very low)
...
function buildReportCardHTML(result, formData, lang, modelIdInfo, operationalRisk) {
  ...
  // v1.11.12: Single source for top score — buildScoreBreakdown.finalScore
  ...
  return `<div id="result-card-inner" style="max-width:560px;margin:0 auto;padding:0 4px">
    ...
    <!-- v1.11.12: Score cap reason — only for truly fatal failures from buildScoreBreakdown -->
    <!-- v1.11.12: capEffective is the ONLY trigger for the failure banner -->
