 ...
 * - https://example.com/openai/v1 → example.com
 * - http://api.example.com → api.example.com
 * - api.example.com → api.example.com
 * - invalid → null (no exception)
 */
function extractHostnameFromBaseUrl(rawBaseUrl) {
  ...
  try {
    // Try to parse as URL, prepend https:// if no protocol
...
/**
 * Guess registrable domain from hostname.
 * - api.example.com → example.com
 * - example.com → example.com
 * - api.foo.example.net → example.net
 */
function guessRegistrableDomain(hostname) {
  if (!hostname) return null;
  const parts = hostname.split('.');
  // Simple heuristic: take last two parts
  // This handles common cases like example.com, foo.example.com
  // For complex TLDs like co.uk, this is a best-effort approach
  if (parts.length >= 2) {
...
/**
 * Build external lookup links for operational risk module.
 */
...
/**
 * Fetch domain registration signal from RDAP.
 * Returns { available, hostname, domainQueried, createdAt, ageDays, source, lookupUrl, error }
 */
async function fetchDomainRegistrationSignal(hostname, signal) {
  const domainToQuery = guessRegistrableDomain(hostname);
  const lookupUrl = `https://rdap.org/domain/${encodeURIComponent(domainToQuery || hostname || '')}`;

  if (!domainToQuery) {
    return { available: false, hostname, domainQueried: null, createdAt: null, ageDays: null, source: 'rdap.org', lookupUrl, error: 'Cannot determine domain to query' };
  }
  ...
  try {
    const resp = await fetch(lookupUrl, { signal: signalToUse, mode: 'cors' });
    ...
    if (data.events && Array.isArray(data.events)) {
      ...
      const regEvent = data.events.find(e =>
        ...
        e.eventAction === 'domain registration' ||
        e.eventAction === 'initial registration'
      );
      if (regEvent && regEvent.eventDate) {
        createdAt = regEvent.eventDate;
      }
    }

    // Fallback: try events array with different formats
    if (!createdAt && data.events && Array.isArray(data.events)) {
      for (const e of data.events) {
        if (e.eventDate && (!createdAt || new Date(e.eventDate) < new Date(createdAt))) {
    ...
    if (!createdAt) {
      return { available: false, hostname, domainQueried: domainToQuery, createdAt: null, ageDays: null, source: 'rdap.org', lookupUrl, error: 'Registration date not found in RDAP response' };
    }
    ...
    return { available: true, hostname, domainQueried: domainToQuery, createdAt, ageDays, source: 'rdap.org', lookupUrl, error: null };
  } catch (err) {
    clearTimeout(timeoutId);
    if (err.name === 'AbortError') {
      return { available: false, hostname, domainQueried: domainToQuery, createdAt: null, ageDays: null, source: 'rdap.org', lookupUrl, error: 'Request timeout' };
    }
    return { available: false, hostname, domainQueried: domainToQuery, createdAt: null, ageDays: null, source: 'rdap.org', lookupUrl, error: err.message || 'Network error' };
  }
}

/**
 * Fetch HTTPS certificate first-seen signal from crt.sh.
 * Returns { available, hostname, firstSeenAt, firstSeenDays, source, lookupUrl, error }
 */
async function fetchCertificateFirstSeenSignal(hostname, signal) {
  if (!hostname) {
    return { available: false, hostname: null, firstSeenAt: null, firstSeenDays: null, source: 'crt.sh', lookupUrl: null, error: 'No hostname provided' };
  }

  const lookupUrl = `https://crt.sh/?q=${encodeURIComponent(hostname)}&output=json`;
  ...
  try {
    ...
    try {
      ...
      // crt.sh sometimes returns non-JSON for some queries
      return { available: false, hostname, firstSeenAt: null, firstSeenDays: null, source: 'crt.sh', lookupUrl, error: 'Failed to parse JSON response' };
    }

    if (!Array.isArray(entries) || entries.length === 0) {
      return { available: false, hostname, firstSeenAt: null, firstSeenDays: null, source: 'crt.sh', lookupUrl, error: 'No certificate records found' };
    }

    // Limit to first 300 entries for performance
    const limitedEntries = entries.slice(0, 300);

    // Find earliest not_before or entry_timestamp
    let earliestDate = null;
    for (const entry of limitedEntries) {
      const notBefore = entry.not_before || entry.entry_timestamp;
      if (notBefore) {
    ...
    if (!earliestDate) {
      return { available: false, hostname, firstSeenAt: null, firstSeenDays: null, source: 'crt.sh', lookupUrl, error: 'No valid timestamp found in certificate records' };
    }

    const firstSeenAt = earliestDate.toISOString();
    const firstSeenMs = Date.now() - earliestDate.getTime();
    const firstSeenDays = Math.floor(firstSeenMs / (1000 * 60 * 60 * 24));

    return { available: true, hostname, firstSeenAt, firstSeenDays, source: 'crt.sh', lookupUrl, error: null };
  } catch (err) {
    clearTimeout(timeoutId);
    if (err.name === 'AbortError') {
      return { available: false, hostname, firstSeenAt: null, firstSeenDays: null, source: 'crt.sh', lookupUrl, error: 'Request timeout' };
    }
    return { available: false, hostname, firstSeenAt: null, firstSeenDays: null, source: 'crt.sh', lookupUrl, error: err.message || 'Network error' };
  }
}

/**
 * Calculate short-term operational risk score and level.
 * Score range: 0-20, independent of API technical score.
 */
  ...
  if (text.startsWith('curl ') || text.includes('curl -')) {
    ...
    return {
      baseUrl: urlMatch ? urlMatch[1]
        .replace(/\/$/, '')
        // Strip path suffixes but keep user's /v1 if present
        .replace(/\/v1\/chat\/(completions|completions\/?)$/i, '/v1')
...
function generateFailureSummary(score, grade, checks) {
  ...
  // Only show "核心响应格式异常" if coreCompatibility is actually failing
  // If coreCompatibility passes (>= 22), don't show this as main risk
  const basicCompatScore = checks.basicCompatibility?.score || 0;
  const coreCompatScore = Math.min(25, basicCompatScore + (checks.targetCall?.score || 0));
  if (!tcResponse || (tcResponse && !tcResponse.choices)) {
    // Only flag if core compatibility is failing
    if (coreCompatScore < 22) {
  ...
  if (modelRisk === 'high' || selfClaimType === 'wrong_family' || selfClaimType === 'hard_contamination') {
    ...
    addReason('MODEL_SIGNAL_REVIEW',
      ...
      zh
        ...
        : `Model family matched but specific version not fully confirmed — manual review recommended`,
      'model');
  }
...
function buildDebugScoring(rawScore, cappedScore, checks, breakdown, extra = {}) {
  ...
  // v1.11.14: Read calibration traces from extra (passed by caller from buildScoreBreakdown)
  const _calibrationTraces = extra._calibrationTraces || null;
  const _basicCalibrationTrace = extra._basicCalibrationTrace || null;
  const _stabilityCalibrationTrace = extra._stabilityCalibrationTrace || null;
  const authEvidence = checks.auth?.evidence || {};
  ...
  return {
    ...
    // v1.11.14: Calibration traces for debugging
    basicCompatibilityCalibrationTrace: _basicCalibrationTrace || null,
    stabilityCalibrationTrace: _stabilityCalibrationTrace || null,
    _calibrationTraces: _calibrationTraces || null,
    ...
    // v1.11.8: Top score and failure banner single source tracking
    topScoreSource: 'buildScoreBreakdown.finalScore',
    failureBannerSource: 'buildScoreBreakdown.capEffective',
    scoreSourceVersion: 'v1.11.14-real-render-source',
    ...
    // Short-term operational risk signals (v1.9) — does NOT affect API score
    operationalRiskEnabled: true,
    operationalRiskAffectsApiScore: false,
    operationalRiskHostname: operationalRisk?.hostname || null,
    operationalRiskDomainQueried: operationalRisk?.domainQueried || null,
    operationalRiskScore: operationalRisk?.score ?? null,
    operationalRiskMax: operationalRisk?.max ?? 20,
    operationalRiskLevel: operationalRisk?.level || 'unknown',
    domainRegistrationAvailable: operationalRisk?.domainRegistration?.available ?? false,
    domainRegistrationCreatedAt: operationalRisk?.domainRegistration?.createdAt ?? null,
    domainRegistrationAgeDays: operationalRisk?.domainRegistration?.ageDays ?? null,
    domainRegistrationSource: operationalRisk?.domainRegistration?.source || null,
    certificateHistoryAvailable: operationalRisk?.certificateHistory?.available ?? false,
    certificateFirstSeenAt: operationalRisk?.certificateHistory?.firstSeenAt ?? null,
    certificateFirstSeenDays: operationalRisk?.certificateHistory?.firstSeenDays ?? null,
...
function calcFinalScore(checks) {
  ...
  // v1.11.16: risk semantic mapping for modelSignal (label only)
  const modelSignalRisk = (() => {
    const severe = new Set(['wrong_family', 'hard_contamination', 'capability_failed', 'severe_mismatch']);
    const mediumCats = new Set(['ambiguous', 'unable_to_confirm', 'no_answer', 'platform_or_proxy_identity', 'partial_match', 'platform_identity']);
    const familyCats = new Set(['family_match', 'likely_same_family']);
    const lowCats = new Set(['exact_match', 'strong_match']);

    if (severe.has(selfClaimType)) return 'high';
    if (mediumCats.has(selfClaimType)) return 'medium';
    if (lowCats.has(selfClaimType)) return 'low';
    if (familyCats.has(selfClaimType)) {
      return (modelSignalScore / 15) >= 0.8 ? 'low' : 'medium';
    }
    // default to medium for unknown identity-like types
    return selfClaimType === 'unknown' ? 'medium' : 'medium';
  })();
  const cacheRisk = cacheScore >= 4 ? 'low' : cacheScore >= 2 ? 'medium' : 'high';
  const clientRisk = clientScore >= 4 ? 'low' : clientScore >= 2 ? 'medium' : 'high';

  return {
    totalScore, // Sum of breakdown scores (displayed number)
    gradeScore: Math.round(gradeScore * 10) / 10, // For grade calculation
...
function buildModuleScores(checks, locale) {
  ...
  }[risk] || { zh: '未验证', en: 'Unverified' });

  // v1.11.16: semantic risk mapping for certain modules (labels only; scores unchanged)
  const riskLevelByModelSignal = (score, max, category) => {
    ...
    if (familyCats.has(cat)) {
      // family_match: medium or low depending on score
      const ratio = max > 0 ? score / max : 0;
      return ratio >= 0.8 ? 'low' : 'medium';
    }
    // fallback: score ratio
    return getRisk(score, max);
  };

  const riskLevelByBasicCompatibility = (score, max, reason, tcEv) => {
    const r = String(reason || '');
    // reason-driven mapping
    ...
    const responseParsed = tcEv?.responseParsed === true;
    const openAICompatible = tcEv?.openAICompatible === true;
  ...
  // ── Module 1: usageTransparency (v1.11.12 calibration) ──
  ...
  // v1.11.12 calibration: A. targetCall failed → 0/25
  if (!realTargetCallSuccess) {
    usageScore = 0;
    usageReason = 'target_call_failed';
    usageSource = 'v11112_calibration: target failed → 0';
  }
  // B. targetCall success + usage missing + timeout → 12/25
    ...
    usageSource = 'v11112_calibration: target success + usage missing → 12';
  }
  // C. targetCall success + partial usage → 16/25
  else {
    ...
    if (hasCacheDetails) {
      // E. complete usage + cache details → 25/25
      usageScore = 25;
      usageReason = 'usage_complete_with_cache_details';
      usageSource = 'v11112_calibration: usage + cache details → 25';
    } else if (usageFields >= 3) {
      // D. complete usage → 21/25
      usageScore = 21;
      usageReason = 'usage_complete';
      usageSource = 'v11112_calibration: complete usage → 21';
    } else {
      // C. partial usage → 16/25
      usageScore = 16;
      usageReason = 'usage_partial';
      usageSource = 'v11112_calibration: partial usage → 16';
    }
  }
  ...
  // ── Module 4: stabilityLatency (v1.11.13 calibration) ──
  ...
  // v1.11.13 calibration: all success + acceptable latency + slight fluctuation → 22/25
  if (totalStabilitySamples >= 5 && okCount === 5) {
    ...
    // All success + acceptable latency (avg < 2000ms) + only slight fluctuation
    ...
    // All success but higher latency or more fluctuation
    else if (avgLat <= 3500 && latencyRatio <= 2.0) {
      // Keep at raw score (likely 20-21)
    }
  }
  const stabilityMax = 25;

  // ── Module 5: basicCompatibility (v1.11.13 calibration) ──
  ...
  const mlCompat = basicEvidence.mlCompat || 0;

  let basicScore = rawBasicScore;
  let basicReason = 'legacy';
  let basicSource = 'checks.basicCompatibility.score';

  // v1.11.15 calibration: distinguish parseable vs OpenAI-compatible
  // responseParsed=true only means valid JSON, NOT OpenAI-compatible.
  // Rules:
  // B) targetCall ok + responseParsed=true but openAICompatible=false → 20/25 (parsed_json_but_nonstandard_schema)
  // C) targetCall ok + responseParsed=true + openAICompatible=true + hasContent=true → 23/25
  // D) (optional future) very complete OpenAI-compatible fields → 25/25 (not enabled in v1.11.15)
  if (realTargetCallSuccess && reachCompat >= 1.5 && authCompat >= 1.5) {
    if (targetCallEvidence.responseParsed === true && targetCallEvidence.openAICompatible !== true) {
      if (rawBasicScore < 20) {
        basicScore = 20;
        basicReason = 'parsed_json_but_nonstandard_schema';
        basicSource = 'v11115_calibration: parsed json but schema non-standard → 20';
      }
    } else if (targetCallEvidence.responseParsed === true && targetCallEvidence.openAICompatible === true && targetCallEvidence.hasContent === true) {
      if (rawBasicScore < 23) {
        basicScore = 23;
        basicReason = 'full_compatibility_passed';
        basicSource = 'v11115_calibration: openai-compatible + content → 23';
      }
    } else if (rawBasicScore < 20) {
      // Parsed but no content extracted (or not parsed)
      basicScore = 20;
      basicReason = 'minor_compatibility_issues';
      basicSource = 'v11115_calibration: minor issues → 20';
    }
  }
  // A) targetCall failed → keep raw score

  const basicMax = 25;

  // ── Module 6: clientConfig (v1.11.12 calibration) ──
  const clientCheck = sc.clientConfig || {};
  const rawClientScore = safeNum(clientCheck.score, 0);
  const clientMax = 5;
  ...
  // v1.11.12 calibration: normalize raw score (0-3) to module score (0-5)
  // raw=3 → module=5 (full config, exportable)
  // raw=2 → module=3 (partial)
  // raw=1 → module=2 (minimal)
  // raw=0 → module=0 (none)
  if (rawClientScore >= 3) {
    clientScore = 5;
    clientReason = 'full_config_exportable';
    clientSource = 'v11112_calibration: raw 3 → 5';
  } else if (rawClientScore >= 2) {
    clientScore = 3;
    clientReason = 'partial_config';
    clientSource = 'v11112_calibration: raw 2 → 3';
  } else if (rawClientScore >= 1) {
    clientScore = 2;
    clientReason = 'minimal_config';
    clientSource = 'v11112_calibration: raw 1 → 2';
  } else {
    clientScore = 0;
    clientReason = 'no_config';
    clientSource = 'v11112_calibration: raw 0 → 0';
  }
  ...
  return [
    ...
    },
    {
      ...
      risk: riskLevelByModelSignal(modelScore, modelMax, selfClaimType),
      riskLabelZh: riskLabel(riskLevelByModelSignal(modelScore, modelMax, selfClaimType)).zh,
      riskLabelEn: riskLabel(riskLevelByModelSignal(modelScore, modelMax, selfClaimType)).en
    },
    {
    ...
    },
    {
      ...
      source: basicSource,
      reason: basicReason,
      risk: riskLevelByBasicCompatibility(basicScore, basicMax, basicReason, targetCallEvidence),
      riskLabelZh: riskLabel(riskLevelByBasicCompatibility(basicScore, basicMax, basicReason, targetCallEvidence)).zh,
      riskLabelEn: riskLabel(riskLevelByBasicCompatibility(basicScore, basicMax, basicReason, targetCallEvidence)).en
    },
    {
      key: 'clientConfig',
      labelZh: '客户端配置',
  ...
  const basicCalibrationTrace = {
    ...
    conditions: {
      ...
      reachCompat: reachCompat,
      reachPass: reachCompat >= 1.5,
      authCompat: authCompat,
      authPass: authCompat >= 1.5,
  ...
  const stabilityCalibrationTrace = (() => {
    ...
    const errorCount = stabilitySamples.filter(s => s && (s.errMsg || s.error || (!s.ok && !s.timeout))).length;
    const http5xxCount = stabilitySamples.filter(s => s && s.status >= 500 && s.status < 600).length;
    ...
    const failed = [];
    if (totalStabilitySamples < 5) failed.push('sampleCount<' + totalStabilitySamples);
    if (okCount !== totalStabilitySamples) failed.push('successCount=' + okCount + '/'+ totalStabilitySamples);
    if (timeoutCount > 0) failed.push('timeoutCount=' + timeoutCount);
    if (errorCount > 0) failed.push('errorCount=' + errorCount);
    if (http5xxCount > 0) failed.push('http5xxCount=' + http5xxCount);
    if (avgLat > 2000) failed.push('avgLat=' + Math.round(avgLat) + 'ms>2000');
    if (latencyRatio > 1.5) failed.push('latencyRatio=' + latencyRatio.toFixed(2) + '>1.5');
    return {
      ...
      samples: stabilitySamples.map((s, i) => ({
        index: i,
        ok: s ? !!s.ok : null,
        timeout: s ? !!s.timeout : null,
        latency: s ? (s.latency || 0) : 0,
        status: s ? (s.status || 0) : 0,
        hasContent: s ? !!s.hasContent : null
      }))
    };
  })();

  // Attach traces to the modules array (returned as array[0]._traces for debug access)
  const basicMod = modules.find(m => m.key === 'coreCompatibility');
  const stabilityMod = modules.find(m => m.key === 'stabilityLatency');
  if (basicMod) { basicMod._calibrationTrace = basicCalibrationTrace; }
  if (stabilityMod) { stabilityMod._calibrationTrace = stabilityCalibrationTrace; }

  // v1.11.14: Also return traces alongside the modules array
  // Return as a plain object so callers expecting [] still get []
  modules._basicCalibrationTrace = basicCalibrationTrace;
  modules._stabilityCalibrationTrace = stabilityCalibrationTrace;
  modules._calibrationTraces = {
    basicCompatibility: basicCalibrationTrace,
    stabilityLatency: stabilityCalibrationTrace
  };
...
function buildScoreBreakdown(checks, locale) {
  ...
  // v1.11.7: Build normalizedEvidence to pass to applyFatalCapsToRaw.
  // This ensures cap decisions use the SAME normalized data (min-20 calibration)
  // that feeds module scores and finalScore.
  // targetCallEvidence mirrors what buildModuleScores built (same sc object).
  const targetCallEvidence = {
    attempted: !!(sc.targetCall),
    ok: sc.targetCall?.ok ?? null,
    httpStatus: sc.targetCall?.evidence?.httpStatus ?? null,
  ...
  const coreCompatModule = modules.find(m => m.key === 'coreCompatibility');
  const basicScore = coreCompatModule?.score ?? (sc.basicCompatibility?.score || 0);

  const normalizedEvidence = { targetCallEvidence, basicScore };
  const capResult = applyFatalCapsToRaw(rawModuleScore, sc, normalizedEvidence);
  ...
  // v1.11.7: Build scorePathTrace for debugging
  const scorePathTrace = {
    version: 'v1.11.7-score-wiring',
  ...
  return {
    ...
    scorePathTrace,
    // v1.11.7: normalized cap input for debugging
    _capNormalized: {
      basicCompatibilityScoreForCap: basicScore,
      targetCallEvidenceForCap: targetCallEvidence
    },
    // v1.11.14: Calibration traces from buildModuleScores
    _calibrationTraces: modules._calibrationTraces || null,
    _basicCalibrationTrace: modules._basicCalibrationTrace || null,
    _stabilityCalibrationTrace: modules._stabilityCalibrationTrace || null
  };
}
...
/* ═══════════════════════════════════════════════════════
   v1.11.7: Fatal cap using normalized evidence
   Only true core failures trigger cap.
   Cap limits are raw scores, not percentages.
   v1.11.7: Uses normalizedEvidence.targetCallEvidence (built in buildModuleScores)
   instead of re-reading raw checks fields. This ensures cap decisions use
   the same normalized data that feeds module scores and finalScore.
   ═══════════════════════════════════════════════════════ */
function applyFatalCapsToRaw(rawScore, checks, normalizedEvidence) {
  let applied = false;
  let reason = null;
  let limit = null;

  // v1.11.7: Use pre-normalized targetCallEvidence when available.
  // normalizedEvidence.targetCallEvidence is built in buildModuleScores using
  // the same sc (checks) object, so it's consistent with module scores.
  // Fall back to inline extraction only for fields not in normalizedEvidence.
  let targetCallEvidence;
  if (normalizedEvidence && normalizedEvidence.targetCallEvidence) {
    targetCallEvidence = { ...normalizedEvidence.targetCallEvidence };
  } else {
    targetCallEvidence = {
  ...
  // v1.11.7: Use normalized basicCompatibility score from buildModuleScores.
  // This is the calibrated score (min 20 when targetCall succeeds),
  // NOT the raw checks.basicCompatibility.score.
  const basicCompatScore = (normalizedEvidence && normalizedEvidence.basicScore !== undefined)
    ? normalizedEvidence.basicScore
    : (checks.basicCompatibility?.score || 0);
  ...
  // v1.11.7: response_format_incompatible can only trigger when:
  // 1. basicCompatibility normalized score < 20 (genuinely low compatibility)
  // 2. realTargetCallSuccess (actual target call worked)
  // 3. response not parsed (format incompatibility)
  // If basicCompatScore >= 20, the response is acceptable — do NOT cap.
  const genuinelyLowCompat = basicCompatScore < 20;
  const canTriggerFormatCap = genuinelyLowCompat &&
    realTargetCallSuccess &&
    !targetCallEvidence.responseParsed;
  ...
  // 5. Response format incompatible — ONLY when:
  //    - basicCompatibility normalized < 20 (genuinely incompatible)
  //    - real targetCall succeeded (not timeout/fallback)
  //    - response was not parseable
  // v1.11.7: basicCompatScore >= 20 → response acceptable, NO cap
  ...
  const capEvidence = {
    ...
    errorType: reason,
    canTriggerFormatCap,
    capSourceVersion: 'v1.11.7-normalized-cap-source'
  };
...
function generateSuggestions(checks, modelIdInfo) {
  ...
  // Priority 4: max_tokens not enforced — change to grey/normal reminder, not main risk
...
/* ═══════════════════════════════════════════════════════
   Report Card HTML Builder — v5 (added operational risk v1.9)
   ═══════════════════════════════════════════════════════ */
function buildReportCardHTML(result, formData, lang, modelIdInfo, operationalRisk) {
  const zh = lang !== 'en';
  const { score: _unusedScore, totalScore, breakdown, checks, reportId, deepMode, toolCallingResult, failureSummary, capApplied, capReason, capLimit, breakdownTotalRaw } = result;

  // Safe accessors for potentially empty checks and breakdown
  const safeChecks = checks || {};
  const safeBreakdown = breakdown || {};

  // v1.11.14: Single source for top score — buildScoreBreakdown.finalScore
  // NEVER use result.score, result.totalScore, or any legacy capped score for top display
  ...
  // v1.11.8: Raw module score from breakdown
  const rawModuleScore = safeBreakdown.rawModuleScore ?? breakdownTotalRaw ?? totalScore;
  ...
  // v1.11.16: source transparency / modelSignal summary risk semantic mapping
  const srcRisk = (() => {
    const severe = new Set(['wrong_family', 'hard_contamination', 'capability_failed', 'severe_mismatch']);
    const mediumCats = new Set(['ambiguous', 'unable_to_confirm', 'no_answer', 'platform_or_proxy_identity', 'partial_match', 'platform_identity']);
    const familyCats = new Set(['family_match', 'likely_same_family']);
    ...
    if (lowCats.has(selfClaimType)) return 'low';
    if (familyCats.has(selfClaimType)) {
      const ratio = (moduleByKey.modelSignal?.max || 15) > 0 ? (safeNum(moduleByKey.modelSignal?.score, 0) / (moduleByKey.modelSignal?.max || 15)) : 0;
      return ratio >= 0.8 ? 'low' : 'medium';
    }
    return 'medium';
  })();
  ...
  return `<div id="result-card-inner" style="max-width:560px;margin:0 auto;padding:0 4px">
    ...
    <!-- v1.11.14: Score cap reason — only for truly fatal failures from buildScoreBreakdown -->
    <!-- v1.11.14: capEffective is the ONLY trigger for the failure banner -->
    <!-- capDetected=true but capEffective=false → do NOT show the banner -->
    ${(() => {
      // v1.11.8: Use breakdown.capEffective as the ONLY source for failure banner visibility
      const capEffective = safeBreakdown.capEffective;
      const capReasonFromBreakdown = safeBreakdown.capReason;
      // Only show if capEffective=true AND capReason is truly fatal
      if (!capEffective || !capReasonFromBreakdown) return '';
      ...
      if (!trulyFatalReasons.includes(capReasonFromBreakdown)) return '';
      const reasonLabels = {
      ...
      const reasonText = reasonLabels[capReasonFromBreakdown] || capReasonFromBreakdown;
      return `<div data-failure-banner data-failure-banner-source="buildScoreBreakdown.capEffective" style="background:#fee2e2;border-radius:8px;padding:6px 12px;margin-bottom:6px;font-size:10px;color:#dc2626;line-height:1.5;border:1px solid #fecaca">${zh ? `⚠ 关键失败封顶：${reasonText}` : `⚠ Capped: ${reasonText}`}</div>`;
    })()}
    ...
    ${(() => {
      ...
      const allUnavailable = !domainAvailable && !certAvailable;
      const domainAgeText = domainAvailable
        ? `${ageDays} ${zh ? '天前' : 'days ago'}`
        : allUnavailable ? (zh ? '未确认' : 'Unconfirmed') : (zh ? '自动查询失败' : 'Auto-lookup failed');

      // v1.11.16: public signals partial evidence → show (partial evidence)
      const scoreText = domainAgeSignal && domainAgeSignal.score !== null
        ? `${domainAgeSignal.score}/10`
        : '';

      const isPartial = (domainAvailable || certAvailable) && !(domainAvailable && certAvailable);
      const partialHint = isPartial ? (zh ? '（部分证据）' : ' (Partial Evidence)') : '';

      // v1.11.16: if any evidence is available, never show "检测失败" here
      const safeAgeLabel = allUnavailable ? (zh ? '未确认' : 'Unconfirmed') : ageLabel;
      ...
      return `
        <div style="background:${lc.cardBg};border:1px solid ${lc.cardBorder};border-radius:8px;padding:8px 10px;margin-bottom:8px">
          <div style="font-size:10px;color:#374151;line-height:1.6">
            <div>
              <span style="font-weight:700;color:${lc.tagColor}">${title}：${safeAgeLabel}${partialHint}${scoreSuffix}</span>
            </div>
...
window.Doctor = {
  ...
  onConnectionInfoInput(textarea) {
    ...
    try {
      ...
      if (parsed.baseUrl) {
        ...
        if (urlEl) urlEl.value = parsed.baseUrl;
      }
      if (parsed.apiKey) {
        const keyEl = document.getElementById('doctor-api-key');
        if (keyEl) keyEl.value = parsed.apiKey;
      }
      // Do NOT clear user's existing Model ID when parsed.model is empty
      if (parsed.model) {
        const modelEl = document.getElementById('doctor-model');
    ...
    try {
      ...
      try {
        debugScoring = buildDebugScoring(rawModuleScore, finalScore, checks, breakdown, {
          ...
          // v1.11.14: pass calibration traces from breakdown
          _calibrationTraces: breakdown._calibrationTraces,
          _basicCalibrationTrace: breakdown._basicCalibrationTrace,
          _stabilityCalibrationTrace: breakdown._stabilityCalibrationTrace
        });
      } catch (err) {
        debugScoring = {
  ...
  showResult(result) {
    ...
    try {
      ...
      console.error('[v1.11.11] buildReportCardHTML failed', buildErr);
      // Capture error for debugScoring
      renderErrorInfo = {
        message: buildErr?.message || String(buildErr),
        name: buildErr?.name || 'Error',
        stack: buildErr?.stack || null
      };
      // Capture result context
      try {
        renderErrorInfo.resultKeys = Object.keys(result || {});
        renderErrorInfo.formDataKeys = Object.keys(this._formData || {});
        renderErrorInfo.modelIdKeys = Object.keys(this._modelIdInfo || {});
        renderErrorInfo.opRiskKeys = Object.keys(result?.operationalRisk || {});
      } catch (e) { /* ignore context capture errors */ }
      ...
      // Show error message but NOT stack or API keys
      const safeErrMsg = (renderErrorInfo.message || '').replace(/\b[A-Za-z0-9_]{20,}\b/g, '***');
      html = `
        <div style="background:#fff;border-radius:16px;padding:24px;text-align:center">
          ...
          ${safeErrMsg ? `<div style="margin-top:12px;font-size:11px;color:#dc2626;max-width:400px;margin-left:auto;margin-right:auto">渲染错误：${esc(safeErrMsg.substring(0, 200))}</div>` : ''}
        </div>
      `;
    }

    resultNode.innerHTML = html;
    // v1.11.9: Verify rendered DOM matches breakdown data
    if (result && result.debugScoring) {
      try {
        const renderedScoreEl = resultNode.querySelector('[data-score-hero-value]');
        const renderedFailureEl = resultNode.querySelector('[data-failure-banner]');
        result.debugScoring.runtimeRenderedScoreText = renderedScoreEl ? renderedScoreEl.textContent : null;
        result.debugScoring.runtimeRenderedFailureText = renderedFailureEl ? renderedFailureEl.textContent : null;
        result.debugScoring.runtimeTopScoreSource = renderedScoreEl ? (renderedScoreEl.getAttribute('data-score-hero-source') || 'unknown') : 'not-found';
        result.debugScoring.runtimeFailureBannerSource = renderedFailureEl ? (renderedFailureEl.getAttribute('data-failure-banner-source') || 'unknown') : 'not-rendered';
      } catch (e) {
        // DOM verification failed - not critical
        result.debugScoring.runtimeRenderError = String(e && e.message ? e.message : e);
      }
      // v1.11.11: Write render error to debugScoring
      if (renderErrorInfo) {
        result.debugScoring.renderError = {
          stage: 'buildReportCardHTML',
          message: renderErrorInfo.message,
          name: renderErrorInfo.name,
          stack: renderErrorInfo.stack,
          version: 'v1.11.11-render-error-capture'
        };
      }
    }
...
const { totalScore, gradeScore } = calcFinalScore(checks);
const capResult = applyCaps(gradeScore, checks, {});
const cappedGrade = extractCappedScore(capResult);
// Convert capped grade score back to sum-based
const capped = capResult?.capApplied ? Math.round((cappedGrade / 100) * totalScore * 10) / 10 : totalScore;
...
const capped = capResult?.capApplied ? Math.round((cappedGrade / 100) * totalScore * 10) / 10 : totalScore;
  ...
  // Case Q: All normal
  caseQ() {
const { totalScore, gradeScore } = calcFinalScore(this._makeNormalChecks());
const capResult = applyCaps(gradeScore, this._makeNormalChecks(), {});
const cappedGrade = extractCappedScore(capResult);
const capped = capResult?.capApplied ? Math.round((cappedGrade / 100) * totalScore * 10) / 10 : totalScore;
